  ...
  return (
        ...
        <div class="flex md:order-2">
            {LoggedIn ? (
                ...
                <Link to='/shopall' ><img src={ShopCart} className='absolute top-1 right-12 w-6 h-6 justify-self-end hover:scale-150'/></Link>  
                <Link to='/shopall' ><img src={ProfileTwo} className='w-8 h-8 justify-self-end hover:scale-150'/></Link>  
                </div>
                </>
                ):(
                <button type="button" class="text-white bg-[#5C5C42] hover:bg-[#FEFAF0] hover:text-[#5C5C42] font-medium rounded-lg text-sm px-4 py-2 text-center mr-3 md:mr-0 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">SIGN UP/ IN</button>
                )}
